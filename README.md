...
| **Asset Type** | Strategy Directive |
| **Control Record** | Strategy Directive Strategy |
...
| POST | `/v1/strategy-directive-strategy/initiate` | Initiate |
| GET | `/v1/strategy-directive-strategy` | Retrieve (list) |
| GET | `/v1/strategy-directive-strategy/{crId}/retrieve` | Retrieve |
| PUT | `/v1/strategy-directive-strategy/{crId}/update` | Update |
| PUT | `/v1/strategy-directive-strategy/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/strategy-directive-strategy/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-corporate-strategy:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-corporate-strategy ./helm -n bian-business-support
```

Exposed through the platform Gateway at path prefix `/sd-corporate-strategy` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
